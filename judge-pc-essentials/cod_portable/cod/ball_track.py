...
def find_ball_candidates(
    ...
    # 와이파이로 압축 스트리밍되는 카메라(Iriun 등)는 화면에 움직임이 있을 때마다 압축 노이즈로
    # 1px 안팎의 얇은 테두리성 잔상이 화면 전체에 수백 개씩 생긴다 — 공 크기와 겹쳐서 그냥
    # dilate만 하면 노이즈가 오히려 더 커진다. erode를 먼저 해서 얇은 노이즈를 지우고(꽉 찬
    # 공 모양은 erode에도 살아남음), 그다음 dilate로 erode가 줄여놓은 진짜 blob 크기를 복원한다.
